        ...
        pokeMarkers.append(
            {
                ...
                'lat': currLat,
                'lng': currLon,
                'infobox': pokemon[1]+' till '+pokemon[-1].strftime('%H:%M:%S')+' @ '+str(pokemon[-3])+', '+str(pokemon[-2])
            })
    fullmap = Map(
        identifier="fullmap",
        style=(
...
def main():
    ...
    access_token = login_ptc("cozymike", "bemyguest")
    ...
    while steps<steplimit:
        ...
        for poke in visible:
            ...
            pokemons.append([poke.pokemon.PokemonId, pokemonsJSON[poke.pokemon.PokemonId - 1]['Name'], poke.Latitude, poke.Longitude, datetime.now() + timedelta(seconds=poke.TimeTillHiddenMs / 1000)])
